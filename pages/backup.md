<!-- Slides dropped from main deck for time. Organized: API first, Types second. -->
...
# API: Backwards compatibility — adding features safely
...
Precise generic signature — the return type *is* the documentation ✅

```python
# aiida/storage/utils.py
T = TypeVar('T')

@singledispatch
def _build_select_stmt(
    dialect: Dialect, coltype: TypeEngine[T], values: Sequence[T]
) -> Select[tuple[T]]: ...
```

`T` enforces that `coltype` and `values` carry the same type, and that the returned `Select` preserves it — no docstring needed.

<v-click>

`CalcInfo` inherits from `DefaultFieldsAttributeDict` — 20+ fields, all implicitly `Optional`, no signature at all ❌
...
Functions returning `None` instead of raising — invisible to callers ❌
...
Untyped utility functions — no contract at all ❌
...
Thanks to @danielhollas for the never-ending quest of adding proper typing throughout `aiida-core`.
...
`NewType` is not used anywhere in aiida-core ❌
